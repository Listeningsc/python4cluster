...
def optimize_affinity_propagation(coords_rad, pca_features, alpha=0.7):
    ...
    sim_matrix = alpha * (-geo_dist) + (1 - alpha) * (-feat_dist)
...
def process_year(year_info):
    ...
    try:
        ...
        scaler   = StandardScaler()
        feats_scaled_ap = scaler.fit_transform(feats_ap.T).T
        pca      = PCA(n_components=0.9)
...
if __name__ == '__main__':
    ...
    output_path = r'/scratch/lijiaxing/output/python_output/cluster/AP4Kcenter/grid_search/Kcenter_No_Geo/hybrid_matrix/AP_Geo_T/sil4simatrix_filter10_extend_pref'
