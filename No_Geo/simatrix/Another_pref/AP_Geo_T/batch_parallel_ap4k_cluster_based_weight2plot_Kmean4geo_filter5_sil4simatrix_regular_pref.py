...
def process_year(year_info):
    ...
    try:
        ...
        feats_ap = features[:,0:10]
        feats_ap_scaled = scaler.fit_transform(feats_ap.T).T
        pca      = PCA(n_components=0.9)
        pca_feats_ap= pca.fit_transform(feats_ap_scaled)
        # 特征及PCA

        scaler   = StandardScaler()
        feats_scaled_kcenter = scaler.fit_transform(features[:,2:10])
        pca      = PCA(n_components=0.9)
...
if __name__ == '__main__':
    ...
    output_path = r'/scratch/lijiaxing/output/python_output/cluster/AP4Kcenter/grid_search/No_Geo/Simatrix/Another_pref/AP_Geo_T/sil4simatrix_f5_r_pref'
