...
def optimize_affinity_propagation(coords_rad, pca_features, alpha=0.7):
    ...
    sim_dist = alpha * (geo_dist**2) + (1 - alpha) * (feat_dist**2)
...
def process_year(year_info):
    ...
    try:
        ...
        # 特征及PCA
        coords_rad = np.radians(features[:, [1,0]])
        # 特征及PCA

        scaler   = StandardScaler()
        feats_scaled_kcenter = scaler.fit_transform(features)
        pca      = PCA(n_components=0.9)
        pca_feats_kcenter= pca.fit_transform(feats_scaled_kcenter)
        ...
        for gid in range(K):
            ...
            if mask.sum() < 5: 
                continue
            res = optimize_affinity_propagation(coords_rad[mask], pca_feats_kcenter[mask])
        ...
        # full_labels, outliers = remove_outliers(full_labels, coords_rad, pca_feats_ap)
        ...
        logging.info(f"{year}: {num_clusters} clusters")

        pd.DataFrame(tie_content[centers]).to_csv(os.path.join(out_dir, f"{year}_AP_centroids.csv"), index=False, header=False)
        for lbl in np.unique(full_labels):
            if lbl < 0: continue
            members = np.where(full_labels==lbl)[0]
            pd.DataFrame(tie_content[members]).to_csv(os.path.join(out_dir, f"{year}_AP_cluster_{lbl}.csv"), index=False, header=False)
        ...
        if np.all(np.isnan(sil_avg)):
            ...
            bi, bj = np.unravel_index(np.nanargmax(sil_avg), sil_avg.shape)
            best_p, best_d, best_sil = percentiles[bi], dampings[bj], sil_avg[bi, bj]

        pd.DataFrame([{"percentile": best_p, "damping": best_d, "silhouette": best_sil}]) \
            .to_csv(os.path.join(out_dir, f"{year}_best_params.csv"), index=False)
...
if __name__ == '__main__':
    ...
    output_path = r'/scratch/lijiaxing/output/python_output/cluster/AP4Kcenter/grid_search/Geo/Hybrid_matrix/Another_pref/AP_Geo/sil4simatrix_f5_r_pref'
    ...
    logging.basicConfig(filename=os.path.join(output_path,'clustering_log.txt'), 
                        level=logging.INFO, 
